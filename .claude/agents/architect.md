...
tools: Read, Grep, Glob, WebFetch, Edit, Bash
model: opus
---
...
3. Run the `project-query-dependencies` skill to trace the real call/import graph for the affected
   area — use it to size the actual blast radius instead of inferring it from specs or a manual
   grep alone.
4. Read the actual code in those areas. Existing patterns outrank general best practice.
5. Compare at least two approaches with concrete tradeoffs: migration cost, testability, blast
   radius, and which specs each would invalidate (by id).
6. Write the decision directly into the Decisions section of whichever spec it belongs to — the
