...
title: Checkout flow
status: merged                # draft or approved in a change; merged once folded in
...
owns:                         # glob(s) this spec is the source of truth for; [] is valid
  - "src/checkout/**"
jira: PROJ-1234               # optional pointer, not a synced mirror
related: [feature.cart, tech]
updated: 2026-01-01           # the day this spec was last confirmed against its code
---
```

`updated:` moves only when the spec is confirmed against its code: folding in, a correction in place, or a
clean check of its criteria against the code.

## Change
Only in a spec file inside a change, right under the title: why the spec is being written or changed,
in a sentence or two — or that the change removes it. When the change folds in, it becomes the Change
...
- **Manual proof:** `Source: Manual`, with an optional description under it — who, where, which build. A
  one-line `Verified: Manual` counts too.
...
- **`spec-sync` warns** when the two disagree either way, and about a test it can't find in its source
  file. A contract is exempt from the no-proof warning: its proof is the feature criteria naming it.
...
## Contracts
Only for work spanning several modules: `.agents/.templates/spec-contract.md`, `owns: []`, the full
...
valid for a hotfix or a code-first reconciliation — written when a change folds in, or alongside the
code outside a change. What qualifies and how to word it: spec-style-rules' *One row, one sentence*.
`jira:` is the origin ticket, not a mirror of its status.
