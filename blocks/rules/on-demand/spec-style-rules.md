...
`.agents/skills/spec-review/reference.md` — read an entry when a line here doesn't settle a case.
...
only state. Resolving it strikes both lines through and adds `- Resolved: see Decisions → "{name}"`. It
...
## Polish — `spec-review --fix` catches these when it runs
...
### No filler
"Simply", "just", "basically", "in order to", "leverage", "robust", "seamless" — delete on sight.
