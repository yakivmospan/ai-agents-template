...
- **The user decides a real choice.** At least two options in one table; the `architect` subagent reads
...
1. Read, if not already read this session, `.agents/rules/on-demand/spec-change-rules.md`, `spec-format-rules.md`, `spec-style-rules.md` and `architecture-rules.md`,
   and the whole change: every spec file, the plan if there is one, and — beside each copy — the spec in
   `.specs/` it copies, so you know what changes.
...
3. Pick the part:
   - **No change yet:** work that changes a guarantee goes to `spec-create` first; a design that changes
     none is agreed in chat (core-rules' *Every task*).
...
Once the spec files are agreed, copy `.agents/.templates/implementation-plan.md` into the change folder,
...
1. **Ground it.** Read the code the change touches, and bring findings the design must act on, not a
   tour. Wide reading goes to the `architect` subagent with a bounded brief: which modules, what to find
   out.
2. **Design.**
   - **One obvious way:** a sentence or two, and say there was no real choice.
   - **A real choice:** the options in one table, on concrete tradeoffs — the `architect` subagent
   ...
   - **A choice someone reading only the code would propose again** is also a Decision, per spec-change-rules'
     *Implementation plan*. **A guarantee that has to change** goes back to the spec files, with
     the user, first.
...
4. **The OK.** Run `spec-sync`, and report its drift with proposed fixes, and its warnings. Ask the user to approve the
   spec files and the plan together, set `status: approved` only on the yes, and stop.
...
1. **Scope:** the tasks the user named, or every unticked one. A draft asked to be built is approved by
   ...
   `spec-sync`. **No plan:** the spec files' criteria are the tasks — proof and checkboxes per spec-change-rules'
   *While building*, and what a person saw goes in the report; when all are checked, run `spec-sync` and say it's
   ready to fold into the specs.
2. **Before code:** read `.agents/rules/on-demand/code-style-rules.md` and the code the tasks touch;
   match its patterns.
3. **For each task, in order:**
   1. Re-read the plan, and any spec file edited since the last task.
   2. Build it. Tests, when wanted, go to `test-writer` / `test_writer`, pointed at the change's spec files, the task's criteria and
...
5. **Every task ticked:** run `spec-sync`, and say it's ready to fold into the specs.
...
Then run `spec-sync`, and report what changed. Continue building — from the first unticked task, at
*Build* step 3 — only when the user's message asks for it — that request approves a draft; otherwise wait.
