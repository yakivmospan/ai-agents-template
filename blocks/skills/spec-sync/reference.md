...
|---|---|---|
| `no frontmatter block`, or `frontmatter missing required key(s)` | a Markdown file under `.specs/` that isn't a spec, or a spec missing `id`/`title` | add the frontmatter, or move the file out of `.specs/` |
| `duplicate id` | copy-paste from a template without editing `id`, or a new spec in a change reusing an existing id | rename one; ids are unique. A change's copy of a spec keeps that spec's id — that's not a duplicate |
...
| `has no \`id:\`` | a spec file missing its frontmatter | give it the id — the existing spec's for a copy, a new one for a new spec |
| `copies .specs/…, but its id … isn't that spec's` | a file named after the wrong path, or a copy whose id was changed | name it after the spec it copies, with that spec's id — or rename it to where the new spec lands |
...
| `` is `merged` but still in a change `` | a change finished but not folded in | fold it in with `spec-merge` |
...
| `has no \`title:\``, or `has no \`parent:\`` | a change spec file with incomplete frontmatter | add it before folding in |
...
| `both create .specs/…` | two open changes each writing the same new spec | fold one in first, then make the other a copy of it |
| `both copy \`<id>\`` | two open changes each holding a copy of one spec | fold one in first, then bring its edits into the other |
| `AC-n is new in both` | two copies of one spec adding the same criterion id | renumber one past the highest across both |
...
| `AC-n lists proof under Verified: but isn't checked — check it if the proof holds`, adding `or leave it while its tests are still to be changed` when it lists tests | proof written down, box left unticked — often just forgotten, or a reworded criterion whose tests a task is changing | check it if the proof holds; a reworded criterion stays unchecked until that task is ticked |
