---
name: spec-sync
description: Use after any edit under .specs/, after moving or deleting code a spec's `owns` glob points at, when asked to "run spec-sync" or "rebuild the index", or when a generated file looks stale or a file's owning spec can't be found — rebuilds .specs/INDEX.md, DECISIONS.md and OPEN-QUESTIONS.md and reports drift and warnings. Not for reviewing spec prose (spec-review).
---
...
- **Possibly stale** — owned code has commits after the spec's `updated:`. `spec-verify` can confirm
  one; its last result shows beside the spec. A broad glob
  shows up often — a reason to ask whether that spec is too broad.
- **Known violations** — `Currently violated:` lines. Each goes false silently once the code is fixed:
  re-check it against the code, and remove the line alongside the fix.
- **Pending changes** — any folder under `changes/` directly holding `specs.<path>.md` files; it folds
  in when its owner says so.
