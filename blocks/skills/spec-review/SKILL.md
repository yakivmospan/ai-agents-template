---
name: spec-review
description: Use when asked to "review this spec", "check the spec for style issues", "is this spec well written", or when the user accepts spec-merge's offer of --fix on the specs a change touched — reviews existing specs against spec-style-rules.md and reports findings, or with --fix applies the high-confidence ones. Not for writing or changing a spec (spec-create), checking structure and drift (spec-sync), or reviewing the spec setup as a whole (project-validate).
---

# Spec Review

`spec-style-rules.md` is applied while a spec is written; nothing re-checks it afterwards, and `spec-sync`
...
Read `.agents/rules/on-demand/spec-style-rules.md` and this skill's `reference.md` fresh — the checklist
is the rule, the reference explains each one with examples. Run `spec-sync`, and take what it reports
...
| Decisions | *What was rejected*; *One owner per decision*; doc comment or spec (`spec-rules.md`'s *Where a decision lives*) |
...
| Anywhere | *The delete test*, *No filler*, *Prose over ceremony*, *Every chapter earns its place*; a leftover `## Solution` heading |
...
Apply the high-confidence findings. Then run `spec-sync`. A style pass changes no behaviour: no Change
