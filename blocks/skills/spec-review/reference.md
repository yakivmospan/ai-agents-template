...
Why each rule in `.agents/rules/on-demand/spec-style-rules.md` exists, with bad and good examples. That
...
platform logger instead of using the shared one reads as a `code-style-rules.md` violation to anyone who
...
in general: `spec-rules.md`'s *Where a decision lives*.)
...
"Simply", "just", "basically", "in order to", "leverage", "robust", "seamless" — they cost words
and carry no information. Delete them on sight.
