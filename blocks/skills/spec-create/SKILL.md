---
name: spec-create
description: Use when asked to "create a spec", "spec out X", "start PROJ-1234", "add a feature", "change how X behaves", "update the spec", "bring the spec up to date with the code", "create a spec from code", when handed new requirements, or when core-rules.md's Every task sends work here — starts or continues a change under .specs/changes/, working out whether it's a new feature, a change to a spec, or code already written — or corrects a stale spec in place. Not for a one-line fix, planning or building (spec-implementation-plan) or folding in (spec-merge).
---
...
what will be true, agreed with the user before any code. Nothing lands under `.specs/` until `spec-merge` — except a
stale spec corrected in place.
...
- **Only the user approves, in chat.** Set `status: approved` only on an explicit yes. For a new spec
  written from code, that yes folds the change in and confirms its unchecked criteria — say so when asking.
...
| A spec covers it, and the requirements are changing | **Change** | a copy of that spec, edited |
| The code already does it, and no spec covers it | **From code** | a new spec describing the code |

Each spec file is shaped per spec-change-rules' *The folder* — `specs.feature.logger.md` for
`.specs/feature/logger.md`.
...
- **"Update the spec", and the direction is unclear** — ask one question first: is the code already
  right? Yes: an existing spec is corrected in place, a missing one is *From code*; no is *Change*.
- **One change can mix them** — a new spec and a copy, or a new spec, written *From code*, for the part no
  spec covers.
- **The spec is already copied into an open change** — continue that change and edit its copy as spec-change-rules'
  *Changing course* says. If it's unclear whether the work belongs there, ask
  whether to add it to that change or wait for that change to fold in.
...
1. **Read** `.agents/rules/on-demand/spec-change-rules.md`, `spec-format-rules.md` and
...
3. **Other open changes** holding a copy of the same spec: read them, and keep criterion ids apart.
4. **Module split.** Work that plausibly spans modules, or where which module owns what is itself a
   question, goes through `spec-implementation-plan`'s grounding before any requirement is written. The user naming
...
- **Copy the spec** from `.specs/` into the folder, read it whole — Decisions and Open questions
  included — and edit the copy as it should read afterwards.
...
- **Criteria** — ids per spec-change-rules' *Changing course*; a changed one is rewritten in place,
  ...
  criterion listed.
- **Intent, Constraints, Public surface** are guarantees, approved with the change. **Decisions, Open
  questions, Pitfalls, References** are records, added whenever they come up. A question this change
...
- **Gather** the code and its tests. For a stale spec: the
  spec whole, and what changed since its `updated:` (`spec-sync`'s *Possibly stale*, then `git log` on
...
- **Before asking for approval, re-read every criterion** — reading from code is exactly where one
...
Then:
- **A stale spec corrected in place** — show the edited criteria and ask once, saying the yes confirms any
  criterion without proof, or reworded, as `Source: Manual`; on the yes, write the edit, a Change history row
  (its ticket, or "No ticket") and `updated:`. Nothing folds in; a no leaves it untouched.
- **From code** — ask for approval, saying the yes confirms any criterion without proof as
  `Source: Manual`; the yes folds the change in through `spec-merge`.
- **A change mixing them** — follows *Ahead of code*: its new part is built before anything folds in.
- **Ahead of code, one obvious way** — ask for approval now. An implementation plan comes only when
  the work is worth splitting (`spec-implementation-plan`).
- **Ahead of code, a real choice to make** — agree the spec files with the user, then continue in
  `spec-implementation-plan`; the approval comes with the plan.

Run `spec-sync`; when asking, report its drift with proposed fixes, and its warnings.
