...
Usage:
    python3 .agents/.scripts/build_setup_map.py [--repo-root <path>] [--check]
...
def tracked(root: Path) -> dict[str, int]:
    paths: list[Path] = [root / "AGENTS.md", root / "CLAUDE.md", root / ".agents/rules/LOADER.md"]
    paths += sorted((root / ".agents/rules/always-on").glob("*.md"))
    paths += sorted((root / ".agents/rules/on-demand").glob("*.md"))
...
def aggregates(root: Path, counts: dict[str, int]) -> dict[str, str]:
    ...
    always_files = ["AGENTS.md", "CLAUDE.md", ".agents/rules/LOADER.md"]
    ...
    always = sum(counts.get(p, 0) for p in always_files) + total(".agents/rules/always-on/") + descriptions
    on_demand = (
        total(".agents/rules/on-demand/")
        + total(".agents/skills/")
    )

    n_always = len(list((root / ".agents/rules/always-on").glob("*.md")))
    n_on_demand = len(list((root / ".agents/rules/on-demand").glob("*.md")))
    ...
    n_specs = len([
        ...
        and "changes" not in p.relative_to(root / ".specs").parts  # a copy being changed isn't another spec
    ])
...
def sidecar(root: Path) -> str | None:
    ...
    return ("// Generated by .agents/.scripts/build_setup_map.py for the map's Local section. Do not edit.\n"
...
def rewrite(html: str, counts: dict[str, int], agg: dict[str, str]) -> tuple[str, list[str]]:
    ...
    for path in counts:
        if path in described:
            continue
        if path.startswith((".agents/rules/", ".agents/skills/")):
            missing.append(
