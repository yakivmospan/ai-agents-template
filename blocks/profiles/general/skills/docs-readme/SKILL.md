---
name: docs-readme
description: Use when asked to "write a README", "document this module", "update the docs", or when writing, reviewing or updating a README or an ARCHITECTURE.md — the repository root, a module, a tool or script folder — including any change that renames a command, path or public entry point one names. Enforces KISS and one structure per kind of file. Not for in-code comments (docs-incode, where installed) or specs (spec-create, where the setup has it).
---
...
4. **Say what readers get wrong** — the symptom they'd see, its cause, the fix. The code can't tell
   them that, which is what makes a README worth reading.
5. **One example, the smallest that works.** More only for genuinely different ways to use it, each
   shown once.
6. **Nothing that goes stale on its own** — no version numbers, dates, counts or "currently" that live
   somewhere else. Point at where they're kept.
7. **No ceremony** — no badges, table of contents, empty License or Contributing sections, or
   headings for their own sake. A table only where prose would scan worse.
8. **One fact per line in a dense step** — a list item carrying several facts becomes sub-bullets,
...
9. **No filler** — "simply", "just", "easy", "powerful", "seamless". Delete on sight.
