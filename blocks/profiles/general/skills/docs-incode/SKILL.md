---
name: docs-incode
description: Use when asked to "add comments", "add KDoc" or "doc comments", "document this function/class", or whenever code being written or edited involves a comment — KDoc, Javadoc, docstrings, header or inline `//` comments — writing, reviewing or requesting one. Enforces KISS, a comment states only what the code alone can't. Not for READMEs or ARCHITECTURE.md (docs-readme, where installed) or specs (spec-create, where the setup has it).
---
