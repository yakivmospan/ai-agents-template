...
description: Write a short handoff document so work can continue in a brand-new conversation with no prior context — and attach whatever files (specs, code, docs) the next session actually needs, rather than trying to restate their contents. ONLY on direct request: "snapshot", "save our progress", "let's continue this later", "sum up where we are", "I need a new chat", or similar. Never proactive, never because a conversation looks long.
---
