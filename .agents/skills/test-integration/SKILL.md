...
class ExampleComponentIntegrationTest {
    ...
    @Test
    fun `given external data updates when orchestrator processes then state changes match data pipeline`() {
        // Given
    ...
    @Test
    fun `given gateway encounters exception when requested then error transitions safely to system state`() {
        // Given
