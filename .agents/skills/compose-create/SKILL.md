...
name: compose-create
description: >
  Use this skill when creating a new Compose screen or component, or when updating an
  existing one. Triggers include: any request to add, create, build, or update a Compose
  screen, a Compose component, a UI layout, or a Composable function. Also use when asked
  about Compose architecture, state hoisting, recomposition optimisation, or stability
  annotations. Do NOT use for writing UI tests — use the compose-ui-tests skill instead.
  Do NOT use for ViewModel logic in isolation — use the unit-tests skill instead.
---

# Create or Update Compose Screen / Component

Applies to any Compose UI — Jetpack Compose or Compose Multiplatform — the patterns below are
platform-agnostic.
