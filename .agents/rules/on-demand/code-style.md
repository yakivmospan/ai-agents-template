...
| New dependency → say so first | Never add one silently. Check `specs/02-tech.md`'s Key libraries first — a dependency that duplicates one already listed there is a different conversation than a genuinely new need. |
